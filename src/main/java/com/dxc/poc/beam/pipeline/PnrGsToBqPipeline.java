...
public class PnrGsToBqPipeline {
    ...
    public static void createAndRunPipeline(GsToBqOptions options) {
        ...
        Pipeline.create(options)
            .apply("Read from file", TextIO.read().from(options.getInputFile()))
            // Converts to POJO or GenericRecord with the ticketNumber field
            .apply("Convert to POJO", ParDo.of(ConverToPojo.class))
            // Removes the field tickerNumber and adds saves encrypted version to "_enc"
            .apply(Deidentifying.propertyFromTo("ticketNumber", "_enc")
                    .withEncryptionDetails("projectId", "appName", "keyRingId", "keyId"))
